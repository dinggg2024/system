  ...
  <div>
    ...
    <div class="card" style="margin-bottom: 10px;">
      <el-table :data="data.tableData" strip @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" align="center" v-if="data.user.role === 'ADMIN'"></el-table-column>
        <el-table-column prop="id" label="序号" width="80" align="center" sortable></el-table-column>
        <el-table-column prop="img" label="图书封面" width="100px">
          <template v-slot="scope">
            <el-image style="width: 40px; height: 40px; display: block" v-if="scope.row.img"
                      :src="scope.row.img" :preview-src-list="[scope.row.img]" preview-teleported></el-image>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="图书名称" width="300px" show-overflow-tooltip/>
        <el-table-column prop="price" label="图书价格" width="80px">
          <template v-slot="scope">
            <span style="color: red">￥ {{scope.row.price}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="isbn" label="ISBN号" width="150px"  show-overflow-tooltip/>
        <el-table-column prop="author" label="图书作者" width="100px"/>
        <el-table-column prop="publisher" label="出版社" show-overflow-tooltip/>
        <el-table-column prop="num" label="剩余数量" show-overflow-tooltip>
          <template v-slot="scope">
            <span v-if="scope.row.num < 3" style="color: red">{{scope.row.num}}</span>
            <span v-else>{{scope.row.num}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="typeName" label="所属分类" width="100px"/>
    ...
    <el-dialog title="图书信息" v-model="data.formVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" label-width="80px"  style="padding: 20px 30px" ref="formRef">
        ...
        <el-form-item label="图书名称" prop="name">
          <el-input v-model="data.form.name" placeholder="图书名称"></el-input>
        </el-form-item>
        <el-form-item label="图书价格" prop="price">
          <el-input v-model="data.form.price" placeholder="图书价格"></el-input>
        </el-form-item>
        <el-form-item label="ISBN号" prop="isbn">
          <el-input v-model="data.form.isbn" placeholder="ISBN号"></el-input>
        </el-form-item>
        <el-form-item label="图书作者" prop="author">
          <el-input v-model="data.form.author" placeholder="图书作者"></el-input>
        </el-form-item>
        <el-form-item label="出版社" prop="publisher">
          <el-input v-model="data.form.publisher" placeholder="出版社"></el-input>
        </el-form-item>
        <el-form-item label="剩余数量" prop="num">
          <el-input v-model="data.form.num" placeholder="剩余数量"></el-input>
        </el-form-item>
        <el-form-item label="所属分类" prop="typeId">
          <el-select v-model="data.form.typeId" style="width: 100%">
            <el-option v-for="item in data.typeData" :label="item.name" :value="item.id" :key="item.id"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
...
const borrow = (row) => {
  ...
  request.post('/borrow/add', borrowData).then(res => {
    if (res.code === '200') {
